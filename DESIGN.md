...
Everything sits under `/api/v1`, with DTOs defined through `nestjs-zod`, OpenAPI generated by the Swagger plugin, and client hooks generated from that by Orval.
...
- **API contract:** nestjs-zod DTOs generate the OpenAPI spec through the Swagger plugin, and Orval turns that into TanStack Query hooks, so the client's types come from the DTOs instead of being written a second time.
